...
const item = PaginationItems[page ]
router.push(item.pageURl);
...
const handleLeftPagination = () => {
    const item = PaginationItems[page - 2 ]
    router.push(item.pageURl);
    setPage(prevState => prevState - 1)
}    
  ...
  return (
        ...
        <div className='mx-auto w-[155px] my-20 flex justify-between items-center'>
            ...
            {PaginationItems.map((item) => (
              ...
              <button className={`cursor-pointer group ${page == PaginationItems.length && ('hidden')}`} onClick={handleRightPagination}> <HiChevronRight color='pink' className='group-hover:scale-125 transition duration-150 ease-in-out' size={24}/> </button>   
       </div>
  )
}
